...
export default function FluidGlass({
  ...
  // Canvas style is placed inline so it can be absolutely positioned by the parent container.
  // Make sure the parent (in App) has position:relative and a visible background behind the canvas.
  return (
    <Canvas
      ...
      gl={{
        alpha: true,
        antialias: true,
        preserveDrawingBuffer: true, // keep the rendered buffer so postprocessing / refraction can sample it reliably
        powerPreference: "high-performance",
      }}
      // ensure the created context has a transparent default background
      onCreated={({ gl }) => {
        // set a transparent clear color once
        gl.setClearColor(new THREE.Color(0x000000), 0);
        // Enable physically correct lights if needed (optional)
        // gl.physicallyCorrectLights = true; // Removed: not available in this three.js version
      }}
...
// Use THREE.Mesh<THREE.BufferGeometry, ZoomMaterial> directly instead of ZoomMesh
...
const ModeWrapper = memo(function ModeWrapper({
  ...
  const ref = useRef<THREE.Mesh>(null!);
  const { nodes } = useGLTF(glb);
  // create an offscreen buffer to render the scene into so the transmission material can sample it
  const buffer = useFBO(undefined, undefined, undefined);
  const { viewport: vp } = useThree();
  const [scene] = useState<THREE.Scene>(() => new THREE.Scene());
  const geoWidthRef = useRef<number>(1);
  ...
  useFrame((state, delta) => {
    ...
    // render the portal scene into the FBO buffer (one-off render target usage)
    gl.setRenderTarget(buffer);
    // ensure we clear with transparent background
    gl.clear(true, true, true);
    gl.render(scene, camera);
    gl.setRenderTarget(null);

    // DO NOT override the global clear color every frame here — we set it once in onCreated
    // gl.setClearColor(0x5227ff, 1); <- removed
  });
  ...
  return (
    <>
      {createPortal(children, scene)}
      {/* display the rendered buffer as a full-screen plane so the transmission material can sample it */}
      <mesh renderOrder={0} position={[0, 0, 0]} scale={[vp.width, vp.height, 1]}>
        <planeGeometry />
        <meshBasicMaterial
          map={buffer.texture}
          transparent
      ...
      {/* the refractive object sits above the plane. pointerEvents are enabled on the mesh by default,
          but since the Canvas has pointerEvents:none in style, the rest of the page won't be blocked.
          If you want to interact with the 3D objects, move pointerEvents to the Canvas or specific overlay. */}
      <mesh
        ref={ref}
        scale={scale ?? 0.15}
        rotation-x={Math.PI / 2}
...
function Images() {
  ...
  useFrame(() => {
    if (!group.current) return;
    // the Image material provided by drei uses a "zoom" uniform in its material
    // we adjust it based on scroll ranges
